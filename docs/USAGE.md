...
**6. NLLB-200 엔진 사용 (200개 언어 지원)**
```bash
python main.py documents/paper.pdf --engine nllb
```
...
`google` 엔진(Google Translate)과 `qwen`, `lfm2`, `lfm2-koen-mt`, `nllb`, `yanolja` 엔진(Local LLM)은 별도의 설정 없이 무료로 사용할 수 있지만, `deepl`, `gemini`, `openai`를 사용하려면 API 키 설정이 필요합니다.
